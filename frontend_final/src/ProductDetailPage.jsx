...
const ProductDetailPage = () => {
  ...
  try {
    ...
    const endpoint = isAuction
      ? `${API_BASE_URL}/wishlists/auction/${item._id}` // You'll need to add this route
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch all dashboard data with CORRECT endpoints
      const responses = await Promise.allSettled([
        fetch(`${API_BASE_URL}/products/my-listings`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${API_BASE_URL}/orders/myorders`, { // Fixed: myorders not my-orders
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        ...
        fetch(`${API_BASE_URL}/tickets/my`, { // Fixed: /my not /my-tickets
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);
      ...
      if (responses[1].status === 'fulfilled' && responses[1].value.ok) {
        ...
        purchases = data.orders || []; // Backend returns { orders: [...] }
      }
      ...
      if (responses[3].status === 'fulfilled' && responses[3].value.ok) {
        ...
        disputes = data.tickets || []; // Backend returns { tickets: [...] }
      }
          ...
          <div className="col-lg-6">
            <div className="card border-0 shadow-sm mb-4">
              <div className="card-body p-4">
                ...
                {/* Seller Info - Updated to show rating */}
                  ...
                  <div className="d-flex align-items-center">
                    ...
                    <div className="flex-grow-1">
                      <div className="d-flex align-items-center justify-content-between mb-1">
                        <h6 className="fw-semibold mb-0">{item.seller?.name}</h6>
                        {/* Add seller rating display */}
                        {item.seller?.rating && (
                          <div className="d-flex align-items-center">
                            <Star size={14} className="text-warning me-1" fill="currentColor" />
                            <span className="small fw-medium">{item.seller.rating.toFixed(1)}</span>
                            <span className="small text-muted ms-1">seller</span>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="row">
                      <div className="col-md-4">
                        <div className="text-center mb-4">
                          ...
                          {/* Enhanced seller rating display */}
                          {item.seller?.rating && (
                            <div className="mb-3">
                              <div className="d-flex align-items-center justify-content-center mb-2">
                                <Star size={20} className="text-warning me-2" fill="currentColor" />
                                <span className="h5 fw-bold mb-0">{item.seller.rating.toFixed(1)}</span>
                              ...
                              <div className="d-flex align-items-center justify-content-center">
                                {[1, 2, 3, 4, 5].map((star) => (
                                  <Star
                                    key={star}
                                    size={16}
                                    className={star <= Math.round(item.seller.rating) ? 'text-warning me-1' : 'text-muted me-1'}
                                    fill={star <= Math.round(item.seller.rating) ? 'currentColor' : 'none'}
                                  />
                                ))}
                              </div>
                              <small className="text-muted">Seller Rating</small>
                            </div>
                          )}
                          ...
                          <div className="row text-center">
                            <div className="col-6">
                              <div className="border-end">
                                <div className="fw-bold text-primary">
                                  {/* You can add listing count here */}
                                  --
                                </div>
                            ...
                            <div className="col-6">
                              <div className="fw-bold text-success">
                                {/* You can add completed sales here */}
                                --
                              </div>
                      ...
                      <div className="col-md-8">
                        <h6 className="fw-semibold mb-3">About this Seller</h6>
                        <p className="text-muted mb-4">
                          {item.seller?.bio || 'No additional information provided by this seller.'}
                        </p>
                        
                        {/* Add UserProfileReviews component if available */}
                        <UserProfileReviews
